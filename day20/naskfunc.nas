...
; this function relays syste interruption to console.c::hrb_api()
...
_asm_hrb_api:
		STI 			  ; CLI will be called AUTOMATICALLY when calling INT, which will disable interruptions
						  ; but that's not we need for putting chars.. so need to enable interruption here
		PUSHAD			  ; buffer register values
		PUSHAD 			  ; PUSHAD is touching ESP, therefore it can be used to pass values to the next line CALL
						  ; we need to use the value in EDX to know which api is being chosen
		CALL 	_hrb_api
		ADD 	ESP, 32
		POPAD
		IRETD
