...
void init_gdtidt(void)
{
	...
	*/
	...
	/* 
	   ...
	   we set 0x40 as applicaion friendly segment( + 0x60) so that application can call it directly
	    without causing any exception
	*/
	set_gatedesc(idt + 0x40, (int) asm_hrb_api, 2 * 8, AR_INTGATE32 + 0x60);
	/*
		register exceptions, when an exception occurs we need to terminate the application 
			that causes the exception and return back to console
		0x0c: stack exception, when something in stack went wrong, like overflowing an array 
		0x0d: general exception, will happen when an application that tries to access system memory
		we need to terminate such 
	*/
