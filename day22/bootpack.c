		...
		if(fifo32_status(&fifo) == 0) {
			...
			*/
			...
			if( i >= 256 && i < 512) {
				...
				/* 
					if when an applicaion is running and shift+f1 is pressed, we terminate it 
					similar to ctrl+c
				*/
				if(i == 256 + 0x3b && key_shift != 0 && task_cons->tss.ss0 !=0) {
					...
					cons_putstr0(cons, "\nBreak(key) :\n");
					io_cli();
					/* changes comsole register value and jump to asm_end_app */
...
void debug(struct SHEET *sheet, char *str) {
	putfonts8_asc_sht(sheet, 10, 10, COL8_FFFFFF, COL8_008484, str, strlen(str));
}
