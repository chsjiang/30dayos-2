...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1) {
	int h, bx, by, vx, vy, shtx0, shty0, shtx1, shty1;
	...
	for(h = h0; h <= h1; h++) {
		...
		/*
			(shtx, shty) is the relative coordinates on sheet
			sht->vx0 + shtx = vx0
			(sht->vx0, sht->vy0) is the upper left cornor of the sheet
		*/
		shtx0 = vx0 - sht->vx0;
		shty0 = vy0 - sht->vy0;
		shtx1 = vx1 - sht->vx0;
		...
		/* we only need to draw the intersection of area to draw and area of the sheet */
		if (shtx0 < 0) { shtx0 = 0; }
		if (shty0 < 0) { shty0 = 0; }
		if (shtx1 > sht->bxsize) { shtx1 = sht->bxsize; }
		if (shty1 > sht->bysize) { shty1 = sht->bysize; }

		for(by = shty0; by < shty1; by++) {
			/* convert sht relative coordinates to absolute coordinates */
			vy = sht->vy0 + by;
			for(bx = shtx0; bx < shtx1; bx++) {
				vx = sht->vx0 + bx;
...
/* udpate the color map, note written to vram yet! */
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0) {
	int h, bx, by, vx, vy, shtx0, shty0, shtx1, shty1;
	unsigned char *buf, *map = ctl->map, sid;
	if(vx0 < 0) {
		vx0 = 0;
	}
	if(vy0 < 0) {
	...
	for(h = h0; h <= ctl->top; h++) {
		...
		*/
		...
		/* we only needs to draw the intersection of area to draw and area of the sheet */
		if (shtx0 < 0) { shtx0 = 0; }
		if (shty0 < 0) { shty0 = 0; }
		if (shtx1 > sht->bxsize) { shtx1 = sht->bxsize; }
		if (shty1 > sht->bysize) { shty1 = sht->bysize; }
